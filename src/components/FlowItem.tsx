...
import { setStarred } from "../data/fetcher";
const styles = (theme) => ({
  root: {
    maxWidth: 360,
    paddingTop: 0,
...
type props_types = {
  ...
  large: boolean;
  serviceName: string;
  classes: any; // material ui things
  onClick: (a: FlowItem_type) => void;
  onStar: (a: boolean) => void;
};
type state_types = {
  starred: boolean;
};
export class FlowItem extends Component<any, state_types> {
  ...
  renderItem(item: FlowItem_type) {
    const checked = item.starred == null ? false : item.starred ? true : false;
          ...
          <ListItemIcon>
            <Checkbox
              ...
              onClick={(e) => {
                ...
                this.props.onStar && this.props.onStar(!item.starred);
                e.stopPropagation();
              }}
            />
          </ListItemIcon>
